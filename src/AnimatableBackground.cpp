...
void BlendedBackground::draw(sf::RenderTarget& target, sf::RenderStates states) const
{
    ...
    if (m_old_image.texture()) {
        ...
        old_bg_sprite.setColor({ 255, 255, 255, static_cast<sf::Uint8>(255 * m_old_image.factor()) });
    ...
    if (m_new_image.texture()) {
        ...
        new_bg_sprite.setColor({ 255, 255, 255, static_cast<sf::Uint8>(255 * m_new_image.factor()) });
