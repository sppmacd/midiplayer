    ...
    : m_reader(m_info, player)
{
    ...
    auto register_physics_config = [&](std::string const& prefix, ParticlePhysics& physics) {
        m_info.register_property(prefix + "_x_drag",
            ...
            [&](Config::ArgumentList const& arglist, double) -> bool {
                physics.x_drag = arglist[0].as_float();
                return true;
            });
        m_info.register_property(prefix + "_temperature_multiplier",
            "How fast particles go up due to temperature",
            { { Config::PropertyType::Float, "value" } },
            [&](Config::ArgumentList const& arglist, double) -> bool {
                physics.temperature_multiplier = arglist[0].as_float();
                return true;
            });
        m_info.register_property(prefix + "_gravity",
        ...
        m_info.register_property(prefix + "_temperature_decay",
            ...
            { { Config::PropertyType::Float, "value" } },
            [&](Config::ArgumentList const& arglist, double) -> bool {
                physics.temperature_decay = arglist[0].as_float();
                return true;
            });
    };

    register_physics_config("dust", m_properties.dust_physics);
    register_physics_config("smoke", m_properties.smoke_physics);
